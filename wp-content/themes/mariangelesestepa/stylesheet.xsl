...
<xsl:when test="sitemap:sitemapindex/sitemap:sitemap/sitemap:loc">Índice de <xsl:value-of select="substring-before($sitemap_loc_slash,'/')"/></xsl:when>
<xsl:when test="sitemap:urlset/sitemap:url/sitemap:loc and substring-before($url_loc_slash,'/')!=''">Sitemap de <xsl:value-of select="substring-before($url_loc_slash,'/')"/></xsl:when>
<xsl:when test="sitemap:urlset/sitemap:url/sitemap:loc">Sitemap de <xsl:value-of select="$url_loc_slash"/></xsl:when>
<xsl:otherwise>Sitemap</xsl:otherwise>
</xsl:choose></title>
<meta name="viewport" content="width=device-width,initial-scale=1,shrink-to-fit=no"/>
<link rel="stylesheet" href="/wp-content/themes/mariangelesestepa/sitemap.css"/>
<link rel="apple-touch-icon-precomposed" sizes="300x300" href="https://master-mari-angeles-estepa.test/wp-content/themes/mariangelesestepa/imagenes/logo.png"/>
<link rel="icon" type="image/png" href="https://master-mari-angeles-estepa.test/wp-content/themes/mariangelesestepa/imagenes/logo.png" sizes="300x300"/>
</head><body>

<div class="container-fluid fixed-top">
	<div class="row">
		<div class="bg-backdrop border border-top-0 border-right-0 border-left-0 col-12 px-0">
			<div class="container"><div class="no-gutters"><div class="col-12"><nav class="bg-transparent breadcrumb font-weight-light h3 my-0 px-0 pb-2 pt-3 px-md-3 pt-lg-5 rounded-0">
<a class="breadcrumb-item" title="Ir al Índice de Sitemaps" href="/sitemap-index.xml"><xsl:choose>
<xsl:when test="sitemap:sitemapindex/sitemap:sitemap/sitemap:loc">Índice de <xsl:value-of select="substring-before($sitemap_loc_slash,'/')"/></xsl:when>
<xsl:when test="sitemap:urlset/sitemap:url/sitemap:loc and substring-before($url_loc_slash,'/')!=''">Sitemap de <xsl:value-of select="substring-before($url_loc_slash,'/')"/></xsl:when>
<xsl:when test="sitemap:urlset/sitemap:url/sitemap:loc">Sitemap de <xsl:value-of select="$url_loc_slash"/></xsl:when>
<xsl:otherwise>Sitemap</xsl:otherwise>
</xsl:choose></a>
<xsl:if test="sitemap:urlset/sitemap:url/xhtml:link"><span class="breadcrumb-item text-secondary">enlaces</span></xsl:if>
<xsl:if test="sitemap:urlset/sitemap:url/news:news"><span class="breadcrumb-item text-secondary">noticias</span></xsl:if>
<xsl:if test="sitemap:urlset/sitemap:url/image:image"><span class="breadcrumb-item text-secondary">imágenes</span></xsl:if>
<xsl:if test="sitemap:urlset/sitemap:url/video:video"><span class="breadcrumb-item text-secondary">vídeos</span></xsl:if>
			</nav></div></div></div>
		</div>
	</div>
</div>
<div class="container">
	<div class="row no-gutters">
		<div class="col-12 mb-3 pt px-md-3">
			<h1 class="font-weight-light h3 my-0 pb-2 pt"><xsl:choose><xsl:when test="count(sitemap:sitemapindex/sitemap:sitemap)&gt;0">Este Sitemap Index contiene <xsl:choose><xsl:when test="count(sitemap:sitemapindex/sitemap:sitemap)&gt;1"><xsl:value-of select="count(sitemap:sitemapindex/sitemap:sitemap)"/> sitemaps</xsl:when><xsl:otherwise>un sitemap</xsl:otherwise></xsl:choose></xsl:when><xsl:when test="sitemap:sitemapindex and count(sitemap:sitemapindex/sitemap:sitemap)=0">Este índice está vacío.</xsl:when><xsl:when test="count(sitemap:urlset/sitemap:url)&gt;0">Este Sitemap contiene <xsl:value-of select="count(sitemap:urlset/sitemap:url)"/> links</xsl:when><xsl:when test="sitemap:urlset and count(sitemap:urlset/sitemap:url)=0">¡El Sitemap está vacío</xsl:when><xsl:otherwise>¡Uy! el Sitemap está incommpleto.</xsl:otherwise></xsl:choose></h1>

			<xsl:apply-templates/>

		</div>
	</div>
	<div class="referring">Sitemap generado automáticamente por <a href="https://master-mari-angeles-estepa.test/" rel="noopener" class="brand">Mª Ángeles Estepa</a></div>
...
<th class="border-0 text-left" scope="col">Enlaces:</th>
<xsl:if test="sitemap:url/sitemap:lastmod"><th class="border-0 text-left text-truncate" scope="col">Última modificación:</th></xsl:if>
<xsl:if test="sitemap:url/sitemap:changefreq"><th class="border-0 text-left text-truncate" scope="col">Frecuencia</th></xsl:if>
<xsl:if test="sitemap:url/sitemap:priority"><th class="border-0 text-left text-truncate" scope="col">Prioridad</th></xsl:if>
</tr>
				</thead><tbody>
